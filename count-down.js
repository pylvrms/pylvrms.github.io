$(document).ready(function() {
    $('[data-countdown]').each(function () {
        ...
        var x = setInterval(function () {
            ...
            dataminutes.html(minutes < 10 ? '0'+minutes:minutes).append('<span class="label">Mins</span>');
            dataseconds.html(seconds < 10 ? '0'+seconds:seconds).append('<span class="label">Secs</span>');

            if(t <= 0){
                clearInterval(x);
                datadays.html('0').append('<span class="label">Days</span>');
                datahours.html('0').append('<span class="label">Hours</span>');
                dataminutes.html('0').append('<span class="label">Mins</span>');
                dataseconds.html('0').append('<span class="label">Secs</span>');
            }
        },1000);
    });
});
